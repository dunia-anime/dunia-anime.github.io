...
(function (global, factory) {
  ...
  //--------------Start Design------------------------------
  var createdeskripsiku = function createdeskripsiku(deskripsiku) {
    if (!deskripsiku) {
      var placedeskripsiku = document.createElement('div');
      placedeskripsiku.className = 'vjs-playlist-deskripsiku vjs-playlist-aktiv';
      return placedeskripsiku;
    }

    var pidess = document.createElement('div');
    pidess.className = 'vjs-playlist-deskripsiku';

    if (typeof deskripsiku === 'string') {
      // simple deskripsikus
      var imgdess = document.createElement('div');
      imgdess.value = deskripsiku;
      imgdess.alt = '';
      pidess.appendChild(imgdess);
    } else {
      // responsive deskripsikus
      // additional variations of a <picture> are specified as
      // <source> elements
      for (var i = 0; i < deskripsiku.length - 1; i++) {
        var _variant = deskripsiku[i];
        ...
        pidess.appendChild(source);
      } // the default version of a <picture> is specified by an <imgdess>


      var variant = deskripsiku[deskripsiku.length - 1];

      var _imgdess = document.createElement('div');

      _imgdess.alt = '';

      for (var _prop in variant) {
        _imgdess[_prop] = variant[_prop];
      }

      pidess.appendChild(_imgdess);
    }
  ...
  var createThumbnail = function createThumbnail(thumbnail) {
    ...
    function (_Component) {
      ...
      _proto.createEl = function createEl() {
        ...
        this.deskripsiku = createdeskripsiku(item.deskripsiku);
        li.appendChild(this.deskripsiku); // Duration
        ...
        this.deskripsiku.appendChild(nowPlayingEl); // Title container contains title and "up next"
  
        var titleContainerEl = document.createElement('div');
        titleContainerEl.className = 'vjs-playlist-title-container';
        this.thumbnail.appendChild(titleContainerEl); // Up next
        this.deskripsiku.appendChild(titleContainerEl); // Up next
        ...
        var judul1 = document.createElement('p');        
        var judulnya = item.judul || this.localize('Untitled Video');
        if (judulnya =='Untitled Video' ) {
        }else {      
        judul1.appendChild(document.createTextNode(judulnya));
        judul1.setAttribute('title', judulnya);
        titleContainerEl.appendChild(judul1);
        }
           // Tempat Edit
        var episodenim = document.createElement('div');        
        var episodenya = item.episode || this.localize('Untitled Video');
        if (episodenya =='Untitled Video' ) {
        }else {      
        episodenim.className = 'vjs-playlist-episode';
        episodenim.appendChild(document.createTextNode(episodenya));
        episodenim.setAttribute('title', episodenya);
        titleContainerEl.appendChild(episodenim);
        }
        ...
        var durasi = document.createElement('p');        
        var time = item.durasivi1 || this.localize('Untitled Video');
        if (time =='Untitled Video' ) {
        }else {      
        durasi.appendChild(document.createTextNode(time));
        durasi.setAttribute('title', time);
        titleContainerEl.appendChild(durasi);
        }

        var deskk = document.createElement('p');
        var desktext = item.deskrip1 || this.localize('Untitled Video');        
        if (desktext =='Untitled Video' ) {
        }else {
        deskk.appendChild(document.createTextNode(desktext));
        deskk.setAttribute('title', desktext);
        titleContainerEl.appendChild(deskk);
        }
        ...
        for (var _i = 0; _i < this.items.length; _i++) {
          ...
          if (_i === currentItem) {
            ...
            if (document.activeElement !== item.el()) {
              ...
              dom.addClass(item.deskripsiku, 'vjs-playlist-now-playing');
            }
